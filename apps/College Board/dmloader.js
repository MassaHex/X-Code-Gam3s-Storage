...
/* ********************************************************************* */
/* Load and combine archive data that is split into archives        */
/* ********************************************************************* */

var NanArchiveLoader = {
    // which files to load
    _files: [],
    _fileIndex: 0,
    // file
    ...
    cleanUp: function() {
        ...
        this._onNanArchiveLoaderCompletedListeners = [];
    ...
    loadArchiveDescription: function(descriptionUrl) {
        FileLoader.load(
            ...
            function (error) { NanArchiveLoader.notifyFileDownloadError(descriptionUrl); },
            function (json) { NanArchiveLoader.onReceiveDescription(json); });
    },
    ...
    downloadPiece: function(file, index) {
        ...
        FileLoader.load(
            ...
            function (loaded, total) {
                ...
                downloaded = loaded;
                NanArchiveLoader._currentDownloadBytes += delta;
                NanArchiveLoader.notifyDownloadProgress();
            },
            function (error) {
                NanArchiveLoader.notifyFileDownloadError(error);
            },
            function (response) {
                ...
                NanArchiveLoader.onPieceLoaded(file, piece);
                NanArchiveLoader.notifyDownloadProgress();
    ...
    runApp: function(appCanvasId, extra_params) {
        ...
        if (Module.hasWebGLSupport()) {
            ...
            if (typeof params["can_not_download_file_callback"] === "function") {
                NanArchiveLoader.addFileDownloadErrorListener(params["can_not_download_file_callback"]);
            }
            // Load and assemble archive
            NanArchiveLoader.addFileLoadedListener(Module.onArchiveFileLoaded);
            NanArchiveLoader.addArchiveLoadedListener(Module.onArchiveLoaded);
            NanArchiveLoader.setFileLocationFilter(params["archive_location_filter"]);
            NanArchiveLoader.loadArchiveDescription('/archive_files.json');
        } else {
            Progress.updateProgress(100, "Unable to start [Redacted], WebGL not supported");
