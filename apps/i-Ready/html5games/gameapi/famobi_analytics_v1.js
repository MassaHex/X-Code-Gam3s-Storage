	...
	var famobi_analytics = {
		...
		'trackStats': function(key, value) {
			return new Promise(function(resolve, reject) {
				...
				//EVENT DISPATCH
				let stats_event = new CustomEvent('famobi_tracking_stats', {detail: {
					success: isValid,
					key: key,
					value: value
				}});
				window.dispatchEvent(stats_event);
		...
		'trackEvent': function(event, params) {
			return new Promise(function(resolve, reject) {
				...
				//EVENT DISPATCH
				let stats_event = new CustomEvent('famobi_tracking_event', {detail: {
					success: isValid,
					event: event,
					params: params
				}});
				...
				*/
				...
				if (famobi_analytics.TRIGGER_SCORE.indexOf(event) > -1) {
					...
					switch(window.famobi_gameID) {
						/*
						case "candy-bubble":
							event = "EVENT_TOTALSCORE";
							params = {totalScore: parseInt(params['levelName'].replace("level", ""))};
							break;
							*/
						case "3d-bowling":
							params['levelName'] = params['levelName'].slice(0, -2);
							break;
						case "maze":
							params['levelName'] = params['levelName'].toLowerCase();
							break;
						case "yatzy-classic":
							if(event == "EVENT_LEVELSCORE") {
								if(params['levelName'] == "1P-2P") {
									return resolve(event, params);
								} else {
									event = "EVENT_TOTALSCORE";
									params = {totalScore: params['levelScore']};
								}
							}
							break;
						case "angry-flappy-wings":
							if(params['levelName'] == "2P") {
								return resolve(event, params);
							}
							params['levelName'] = window.famobi.config.level_key;
							break;
						case "perfect-piano":
							if(event == "EVENT_LEVELSCORE") {
								var levelIndex = window.levelNames ? window.levelNames.indexOf(params['levelName']) : -1;
								if(levelIndex >= 0) {
									params['levelName'] = "level" + (levelIndex + 1);
								}
							}
							break;
						case "knife-rain":
					...
					if(!window.famobi.hasFeature("highscores") || (event === "EVENT_TOTALSCORE" && !window.famobi.config.level_key)) {
						return resolve(event, params);
					}
					...
					switch(event) {
						case "EVENT_LEVELSCORE":
							promises.push(api.postScore(params['levelName'], parseInt(params['levelScore'])));
							break;
						case "EVENT_TOTALSCORE":
							promises.push(api.postScore(window.famobi.config.level_key, parseInt(params['totalScore'])));
							break;
						default:
					}
					...
					return Promise.all(promises).then(function() {
						...
						if(window.famobi.config.skipDialog == event || (window.famobi_gameID == "knife-rain" && typeof params['reason'] == "undefined")) {
							resolve(event, params);
							return;
						}

						api.on('DIALOG_CLOSED', function() {
							resolve(event, params);
						});
						...
						setTimeout(function() {
							...
							api.showDialog('scores', {
								title: window.famobi.config.showDialog.title,
								level_key: params['levelName'] || window.famobi.config.level_key,
								period: window.famobi.config.showDialog.period,
								value_formatter: window.famobi.config.showDialog.value_formatter
							});
				...
				if (famobi_analytics.TRIGGER_LEVELFAIL.indexOf(event) > -1) {
					...
					return Promise.all([
						api.endGame({reason: params.reason})
					]).then(function(){
						window.famobi.showAd(function() {
							resolve(event, params);
						});
		...
		'trackScreen': function(screen, pageTitle) {
			//EVENT DISPATCH
			let stats_screen = new CustomEvent('famobi_tracking_screen', {detail: {
				success: true,
				screen: screen,
				pageTitle: pageTitle
			}});
			window.dispatchEvent(stats_screen);
			return new Promise(function(resolve, reject) {
